...
IF "%JAVA_HOME%" == "" (
    for /f "tokens=*" %%i in ('dataloader-50.0.0-java-home.exe') do (
        IF EXIST "%%i" (
            set JAVA_HOME=%%i
        ) ELSE (
            echo %%i
...
IF "%JAVA_HOME%" == "" (
    ...
    IF NOT EXIST "%JAVA_HOME%" (
        ...
        "%JAVA_HOME%\bin\java" -cp ..\dataloader-50.0.0-uber.jar -Dsalesforce.config.dir=%1 com.salesforce.dataloader.process.ProcessRunner %PROCESS_OPTION%
    )
)
